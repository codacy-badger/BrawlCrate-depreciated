...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class TBCLNode : ARCEntryNode, IAttributeList {
        ...
        protected override string GetName() {
            return base.GetName(BrawlLib.Properties.Resources.TBCL);
        }
        ...
        private static void ReadConfig()
        {
            ...
            {
                if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower() + '\\')) + "InternalDocumentation" + "\\TBCL"))
                {
                    foreach (string path in Directory.EnumerateFiles(AppDomain.CurrentDomain.BaseDirectory + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower() + '\\')) + "InternalDocumentation" + "\\TBCL", "*.txt"))
                    {
