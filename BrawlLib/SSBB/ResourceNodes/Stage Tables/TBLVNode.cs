...
namespace BrawlLib.SSBB.ResourceNodes
{
    ...
    {
        public override ResourceType ResourceType => ResourceType.TBLV;
        ...
        [Category("TBLV")] public int NumEntries => entries.Length / 4;
        ...
        {
            return ((TBLV*) source.Address)->_tag == TBLV.Tag ? new TBLVNode() : null;
        }
        ...
        public IEnumerable<AttributeInterpretation> GetPossibleInterpretations()
        {
            ...
            IEnumerable<AttributeInterpretation> q = from f in TBLVFormats
        ...
        private AttributeInterpretation GenerateDefaultInterpretation()
        {
            ...
            string filename = AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation" + "\\TBLV\\" +
                              root.Name.Replace("STG", "") + temp + ".txt";
            return new AttributeInterpretation(arr, filename);
        }

        private static readonly List<AttributeInterpretation> TBLVFormats = new List<AttributeInterpretation>();
            ...
            if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation"))
            {
                if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation" + "\\TBLV"))
                {
                    foreach (string path in Directory.EnumerateFiles(
                        AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation" + "\\TBLV", "*.txt"))
                    {
