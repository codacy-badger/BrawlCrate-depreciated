...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class TBLVNode : ARCEntryNode, IAttributeList
    {
        ...
        private AttributeInterpretation GenerateDefaultInterpretation()
        {
            ...
            if (root != this)
                temp = "[" + FileIndex + "]";
            string filename = AppDomain.CurrentDomain.BaseDirectory + "\\StageDocumentation" + "\\TBLV\\" + root.Name.Replace("STG", "") + temp + ".txt";
        ...
        private static void ReadConfig()
        {
            ...
            {
                if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "\\StageDocumentation" + "\\TBLV"))
                {
                    foreach (string path in Directory.EnumerateFiles(AppDomain.CurrentDomain.BaseDirectory + "\\StageDocumentation" + "\\TBLV", "*.txt"))
                    {
                        if (configpaths_read.Contains(path)) continue;
                        configpaths_read.Add(path);
                        try
                        {
                            TBLVFormats.Add(new AttributeInterpretation(path));
                        }
