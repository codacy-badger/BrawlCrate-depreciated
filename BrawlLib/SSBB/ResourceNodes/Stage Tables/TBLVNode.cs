...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class TBLVNode : ARCEntryNode, IAttributeList
    {
        ...
        private static void ReadConfig()
        {
            ...
            {
                if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower() + '\\')) + "InternalDocumentation" + "\\TBLV"))
                {
                    foreach (string path in Directory.EnumerateFiles(AppDomain.CurrentDomain.BaseDirectory + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower() + '\\')) + "InternalDocumentation" + "\\TBLV", "*.txt"))
                    {
