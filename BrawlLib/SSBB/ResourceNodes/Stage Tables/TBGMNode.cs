...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class TBGMNode : ARCEntryNode, IAttributeList
    {
        ...
        public TBGMNode(VoidPtr address, int numEntries)
        {
            unk0 = 9;               // Default set in Norfair
            unk1 = 0;
            unk2 = 0;
        ...
        {
            return base.GetName(BrawlLib.Properties.Resources.TBGM);
        }

        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            TBGM* header = (TBGM*)address;
            header->_tag = TBGM.Tag;
            header->_unk0 = unk0;
            header->_unk1 = unk1;
            header->_unk2 = unk2;
        ...
        private static void ReadConfig()
        {
            ...
            {
                if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation" + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : ('\\' + System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower())) + "\\TBGM"))
                {
                    foreach (string path in Directory.EnumerateFiles(AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation" + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : ('\\' + System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower())) + "\\TBGM", "*.txt"))
                    {
                        if (configpaths_read.Contains(path)) continue;
                        configpaths_read.Add(path);
                        try
                        {
                            TBGMFormats.Add(new AttributeInterpretation(path, 0x10));
                        }
