...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class STDTNode : ARCEntryNode, IAttributeList {
		...
		private AttributeInterpretation GenerateDefaultInterpretation()
        {
            ...
            if (root != this)
                temp = "[" + FileIndex + "]";
            string filename = AppDomain.CurrentDomain.BaseDirectory + "\\StageDocumentation" + "\\STDT\\" + root.Name.Replace("STG", "") + temp + ".txt";
		...
		private static void ReadConfig()
        {
            ...
            {
                if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "\\StageDocumentation" + "\\STDT"))
                {
                    foreach (string path in Directory.EnumerateFiles(AppDomain.CurrentDomain.BaseDirectory + "\\StageDocumentation" + "\\STDT", "*.txt"))
                    {
