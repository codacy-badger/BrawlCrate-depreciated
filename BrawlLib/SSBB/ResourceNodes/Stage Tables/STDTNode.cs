...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class STDTNode : ARCEntryNode, IAttributeList
    {
        ...
        [Category("Stage Trap Data Table")] public int NumEntries => entries.Length / 4;
        ...
        private AttributeInterpretation GenerateDefaultInterpretation()
        {
            ...
            for (int i = 0; i < arr.Length; i++)
            {
                         ...
                         p.R == 0 && p.G == 50 && p.B == 0)
                {
                    ...
                    if (root.Name.Equals("STGPLANKTON", StringComparison.OrdinalIgnoreCase) &&
                        (p.R % 5 == 0 || p.R % 3 == 0) && (p.B % 5 == 0 || p.B % 3 == 0) &&
            ...
            string filename = AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation" + "\\STDT\\" +
            ...
            if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation"))
            {
                if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation" + "\\STDT"))
                {
                    foreach (string path in Directory.EnumerateFiles(
                        AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation" + "\\STDT", "*.txt"))
                    {
