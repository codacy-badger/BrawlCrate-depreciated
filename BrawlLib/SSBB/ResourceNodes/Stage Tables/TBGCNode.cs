...
{
    public unsafe class TBGCNode : ARCEntryNode, IAttributeList
    {
        public override ResourceType ResourceType { get { return ResourceType.TBGC; } }
        internal TBGC* Header { get { return (TBGC*)WorkingUncompressed.Address; } }
        ...
        public TBGCNode(VoidPtr address, int numEntries)
        {
            unk0 = 25;              // Default set in Smashville
            unk1 = 0;
            unk2 = 0;
        ...
        {
            return base.GetName(BrawlLib.Properties.Resources.TBGC);
        }
        ...
        internal static ResourceNode TryParse(DataSource source) { return ((TBGC*)source.Address)->_tag == TBGC.Tag ? new TBGCNode() : null; }
        [Browsable(false)]
        ...
        private AttributeInterpretation GenerateDefaultInterpretation()
        {
            ...
            if (root != this)
                temp = "[" + FileIndex + "]";
            string filename = AppDomain.CurrentDomain.BaseDirectory + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower() + '\\')) + "InternalDocumentation" + "\\TBGC\\" + root.Name.Replace("STG", "") + temp + ".txt";
            return new AttributeInterpretation(arr, filename);
        }

        private static List<AttributeInterpretation> TBGCFormats = new List<AttributeInterpretation>();
        private static HashSet<string> configpaths_read = new HashSet<string>();

        private static void ReadConfig()
        {
            if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower() + '\\')) + "InternalDocumentation"))
            {
                if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower() + '\\')) + "InternalDocumentation" + "\\TBGC"))
                {
                    foreach (string path in Directory.EnumerateFiles(AppDomain.CurrentDomain.BaseDirectory + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower() + '\\')) + "InternalDocumentation" + "\\TBGC", "*.txt"))
                    {
                        if (configpaths_read.Contains(path)) continue;
                        configpaths_read.Add(path);
                        try
                        {
                            TBGCFormats.Add(new AttributeInterpretation(path, 0x10));
                        }
