...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class TBRMNode : ARCEntryNode, IAttributeList
    {
        ...
        {
            return base.GetName(BrawlLib.Properties.Resources.TBRM);
        }
        ...
        private static void ReadConfig()
        {
            ...
            {
                if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation" + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : ('\\' + System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower())) + "\\TBRM"))
                {
                    foreach (string path in Directory.EnumerateFiles(AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation" + (System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).Equals("en", StringComparison.OrdinalIgnoreCase) ? "" : ('\\' + System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2).ToLower())) + "\\TBRM", "*.txt"))
                    {
                        ...
                        try
                        {
                            TBRMFormats.Add(new AttributeInterpretation(path, 0x10));
                        }
