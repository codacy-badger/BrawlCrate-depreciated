...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class TBRMNode : ARCEntryNode, IAttributeList
    {
        ...
        private AttributeInterpretation GenerateDefaultInterpretation()
        {
            ...
            if (root != this)
                temp = "[" + FileIndex + "]";
            string filename = AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation\\" +  System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2) + "\\TBRM\\" + root.Name.Replace("STG", "") + temp + ".txt";
        ...
        private static void ReadConfig()
        {
            ...
            {
                if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation\\" +  System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2) + "\\TBRM"))
                {
                    foreach (string path in Directory.EnumerateFiles(AppDomain.CurrentDomain.BaseDirectory + "InternalDocumentation\\" +  System.Threading.Thread.CurrentThread.CurrentUICulture.ToString().Substring(0, 2) + "\\TBRM", "*.txt"))
                    {
