...
namespace Net
{
    ...
    {
        public static readonly string mainRepo = "libertyernie/brawltools";
        ...
        public static async Task UpdateCheck(bool Overwrite, string openFile = null, bool Documentation = false, bool Automatic = false)
        {
            ...
            try
            {
                ...
                GitHubClient github = new GitHubClient(new Octokit.ProductHeaderValue("Brawltools")) { Credentials = cr };

                // get Release
                IReadOnlyList<Release> releases = await github.Repository.Release.GetAll("libertyernie", "brawltools");
                if (!Documentation)
                ...
                ReleaseAsset Asset = (await github.Repository.Release.GetAllAssets("libertyernie", "brawltools", release.Id))[0];
                if(Asset == null)
                    return;
                
                if (Overwrite && !Documentation)
                {
                    //Find and close the brawltools application that will be overwritten
                    TRY_AGAIN:
                    Process[] px = Process.GetProcessesByName("BrawlBox");
                    Process[] pToClose = px.Where(x => x.MainModule.FileName.Equals(AppPath + "\\BrawlBox.exe")).ToArray();
                    Process p = px.FirstOrDefault(x => x.MainModule.FileName.Equals(AppPath + "\\BrawlBox.exe"));
                    if (p != null && p != default(Process) && px != null && pToClose != null && pToClose.Length > 1)
                    {
                        DialogResult continueUpdate = MessageBox.Show("Update cannot proceed unless all open windows of " + AppPath + "\\BrawlBox.exe are closed. Would you like to force close all open brawltools windows at this time?\n\n" +
                            "Select \"Yes\" if you would like to force close all open brawltools windows\n" +
                    ...
                    if (Process.GetProcessesByName("winlogon").Count<Process>() == 0 || Documentation || !Overwrite)
                    {
                        try
                        {
                            ...
                            if (Documentation)
                            {
                                ...
                                MessageBox.Show("Documentation was successfully updated to " + ((releases[0].Name.StartsWith("brawltools Documentation", StringComparison.OrdinalIgnoreCase) && releases[0].Name.Length > 26) ? releases[0].Name.Substring(25) : releases[0].Name) + (Automatic ? "\nThis documentation release:\n" + releases[0].Body : ""));
                            }
                        }
                        catch (Exception e)
                        {
        ...
        public static async Task CheckUpdates(string releaseTag, string openFile, bool manual = true, bool checkDocumentation = false, bool automatic = false)
        {
            ...
            try
            {
                var github = new GitHubClient(new Octokit.ProductHeaderValue("Brawltools")) { Credentials = cr };
                IReadOnlyList<Release> AllReleases = null;
                AllReleases = await github.Repository.Release.GetAll("libertyernie", "brawltools");
                ...
                if (releases != null &&
                    ...
                    releases[0].Name.IndexOf("BrawlBox v", StringComparison.InvariantCultureIgnoreCase) >= 0) //Make sure this is a brawltools release
                {
                    if (automatic)
                    {
            ...
            try
            {
                ...
                using (WebClient client = new WebClient())
                {
                    ...
                    using (var sw = new StreamWriter(AppPath + "/Update.bat"))
                    {
                        ...
                        sw.WriteLine("START /wait temp.exe -y");
                        sw.WriteLine("del temp.exe /s /f /q");
                        sw.Write("START BrawlBox.exe \"" + (openFile != null && openFile != "<null>" ? openFile : "null") + "\"");
                        sw.Close();
                    }
                    Process updateBat = Process.Start(new ProcessStartInfo()
                    {
        ...
        public static async Task ForceDownloadCanary(string openFile, string commitID = null)
        {
            ...
            try
            {
                ...
                using (WebClient client = new WebClient())
                {
                    ...
                    //client.DownloadFile(URL, AppPath + "/temp.exe");
                    DLProgressWindow.finished = false;
                    DLProgressWindow dlTrack = new DLProgressWindow(null, commitID == null ? "brawltools Canary Build" : (currentRepo == mainRepo ? (currentBranch == mainBranch ? "brawltools Canary #" + commitID : "Canary@" + currentBranch + " #" + commitID) : currentRepo + " Canary@" + currentBranch + " #" + commitID), AppPath, URL);
                    while (!DLProgressWindow.finished)
                    {
                        // do nothing
                    }
                ...
                using (WebClient client = new WebClient())
                {
                    ...
                    try
                    {
                        Process update = Process.Start(AppPath + "/temp.exe", "-o\"" + AppPath + "\"" + " -y");
                            ...
                            if (File.Exists(AppDomain.CurrentDomain.BaseDirectory + '\\' + "temp.exe"))
                                File.Delete(AppDomain.CurrentDomain.BaseDirectory + '\\' + "temp.exe");
                            MessageBox.Show("Documentation was successfully updated to " + ((releases[0].Name.StartsWith("brawltools Documentation", StringComparison.OrdinalIgnoreCase) && releases[0].Name.Length > 26) ? releases[0].Name.Substring(25) : releases[0].Name) + (true ? "\nThis documentation release:\n" + releases[0].Body : ""));
                    }
                    catch (Exception e)
                    {
                        MessageBox.Show("Error: " + e.Message);
        ...
        public static async Task WriteCanaryTime()
        {
            try
            {
                ...
                try
                {
                    branch = await github.Repository.Branch.Get("libertyernie", "brawltools", currentBranch);
                    result = await github.Repository.Commit.Get("libertyernie", "brawltools", branch.Commit.Sha);
                    commitDate = result.Commit.Author.Date;
                }
                catch
                {
                    branch = await github.Repository.Branch.Get("libertyernie", "brawltools", mainBranch);
                    result = await github.Repository.Commit.Get("libertyernie", "brawltools", branch.Commit.Sha);
                    commitDate = result.Commit.Author.Date;
                    currentBranch = mainBranch;
                }
        ...
        public static async Task ShowCanaryChangelog()
        {
            ...
            if(newSha == oldSha)
            {
                MessageBox.Show("Welcome to brawltools Canary! You were already on the latest commit.");
                if (File.Exists(Filename))
                    File.Delete(Filename);
                return;
            }
            if(newRepo != oldRepo)
            {
                MessageBox.Show("Welcome to brawltools Canary! You are now tracking the " + newBranch + " branch of the " + newRepo + " repository instead of the " + oldBranch + " branch of the " + oldRepo + " repository. Canary changelog is not supported when switching repositories, so please check online to see differences.");
                if (File.Exists(Filename))
                    File.Delete(Filename);
                return;
            }
            ...
            {
                MessageBox.Show("Welcome to brawltools Canary! You are now tracking the " + newBranch + " branch instead of the " + oldBranch + " branch. Canary changelog is not supported when switching branches, so please check the Discord for what's been changed.");
                if (File.Exists(Filename))
                    File.Delete(Filename);
                return;
            }
            ...
            try
            {
                ...
                {
                    //var c = await github.Repository.Commit.Get("libertyernie", "brawltools", branch.Commit.Sha);
    ...
    public static class BugSquish
    {
            ...
            string Description)
        {
            ...
            try
            {
                ...
                if (!TagName.StartsWith("brawltools Canary", StringComparison.OrdinalIgnoreCase))
                {
                    try
                    {
                        releases = await github.Repository.Release.GetAll("libertyernie", "brawltools");

                        // Remove all pre-release (Documentation) versions from the list
                        releases = releases.Where(r => !r.Prerelease).ToList();

                        issues = await github.Issue.GetAllForRepository("BrawlBox", "BrawlBoxIssues");
                    }
                    catch (System.Net.Http.HttpRequestException)
                    {
                        MessageBox.Show("Unable to connect to the internet.");
